	...
	public class ScopeScript : MonoBehaviour{
		...
		void Update() {
			...
			if (crosshair.activeSelf) {
				...
				if (zoom_indicator_background.activeSelf) {
					Color.RGBToHSV(Plugin.indicator_background_color.Value, out float H, out float S, out float V);
			...
			if (zoom_indicator.activeSelf) {
				Color.RGBToHSV(Plugin.indicator_color.Value, out float H, out float S, out float V);
			...
			if (lah.character_input.GetAxis(36) > 0 && camera.fieldOfView != max_zoom) {
				...
				AudioManager.PlayOneShotAttached("event:/Iskra2/Scope/click", gameObject, 0.2f);
			}
