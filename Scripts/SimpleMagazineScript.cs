...
namespace Iskra2Patch {
	public class SimpleMagazineScript : StorableMonoBehaviour {
		...
		internal readonly int max_rounds = 6;
		private readonly float round_move = 0.0055f;

		private Iskra2 iskra;

		public GameObject round_prefab;
		public int queue_rounds = 0;

		private bool extracting;
		private Stack<ShellCasingScript> rounds = new Stack<ShellCasingScript>();
		...
		void Update() {
			if (round_insert_amount != 1) {
				...
				round_insert_amount = Mathf.MoveTowards(round_insert_amount, 1, Time.deltaTime * 7 * Time.timeScale + (-0.1f * num_rounds * Time.deltaTime));
			}
				...
				if (distance < 0){
					for (int i = 0; i < num_rounds; i++) {
						...
						round.transform.localPosition = new Vector3(
							...
							Mathf.MoveTowards(round.transform.localPosition.z, round_top_left.localPosition.z - (i * round_move), Time.deltaTime * Time.timeScale * (num_rounds * 0.1f))
						);
                    }
				}
